...
// ✅ v3.26: HandleDamage - Spawn protection only
_unit addEventHandler ["HandleDamage", {
    params ["_unit", "_selection", "_damage", "_source", "_projectile", "_hitIndex", "_instigator", "_hitPoint"];

    // ✅ v3.24: ABSOLUTE SPAWN PROTECTION - Block ALL damage during protection
    private _spawnProtected = _unit getVariable ["A3XAI_spawnProtected", false];
    if (_spawnProtected) exitWith {
        0  // Return 0 damage - completely block during spawn protection
    };

    // Only log significant damage for debugging (> 0.1, first 30 seconds)
    if (_damage > 0.1) then {
        private _spawnTime = _unit getVariable ["A3XAI_spawnTime", 0];
        private _aliveTime = time - _spawnTime;
        ...
        if (_aliveTime < 30) then {
            ...
            diag_log format ["[A3XAI:DAMAGE] Unit hit at %1s | Damage: %2 | Source: %3 (%4) | Projectile: %5",
                _aliveTime toFixed 1,
                _damage toFixed 2,
                _sourceType,
                _sourceSide,
