...
for "_g" from 0 to (_groupCount - 1) do {
    ...
    if (isNull _group) then {
        [1, format ["Invasion group %1 SKIPPED: createGroup returned null - group limit reached", _g]] call A3XAI_fnc_log;
        continue;
    };
...
if (isNull _defenderGroup) then {
    [1, "Invasion defenders SKIPPED: createGroup returned null - group limit reached"] call A3XAI_fnc_log;
} else {
...
if (false) then {  // ✅ v3.20: Disabled to keep AI at 5 max
    ...
    if (isNull _sniperGroup) then {
        [1, "Invasion snipers SKIPPED: createGroup returned null - group limit reached"] call A3XAI_fnc_log;
    } else {
        ...
        if (_sniperGroupSide != EAST) then {
            ...
            for "_s" from 0 to ((_sniperCount - 1) min (count _tallBuildings - 1)) do {
                ...
                _sniper setDamage 0;
                    ...
                    _sniper setVariable ["A3XAI_spawnProtected", true, true];
