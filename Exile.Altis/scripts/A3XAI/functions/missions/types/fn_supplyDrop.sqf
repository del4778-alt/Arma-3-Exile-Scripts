...
if (isNull _pilotGroup) exitWith {
    [1, format ["SupplyDrop '%1' FAILED: createGroup (pilot) returned null - group limit reached", _missionName]] call A3XAI_fnc_log;
    deleteVehicle _heli;
    createHashMap
};
...
if (isNull _paraGroup) exitWith {
    [1, format ["SupplyDrop '%1' FAILED: createGroup (para) returned null - group limit reached", _missionName]] call A3XAI_fnc_log;
    createHashMap
};
...
for "_i" from 0 to (_paradropCount - 1) do {
    ...
    _para setVariable ["A3XAI_spawnProtected", true, true];
