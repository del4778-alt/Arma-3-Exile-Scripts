...
if (isNil "PATROL_FactionsConfigured") then {
    ...
    // 🔥 v8.4: RESISTANCE (patrols) friendly to INDEPENDENT (recruit AI)
    SIDE_RES setFriend [INDEPENDENT, 1];
    INDEPENDENT setFriend [SIDE_RES, 1];
    
    // RESISTANCE friendly to CIVILIAN (players)
    SIDE_RES setFriend [CIVILIAN, 1];
    CIVILIAN setFriend [SIDE_RES, 1];
...
DEFENDER_fnc_setSkills = {
    ...
    if (PATROL_VCOMAI_Active) then {
        ...
        _u setVariable ["PatrolAI_unit", true, true];  // v8.4: Mark as friendly patrol
...
DEFENDER_fnc_combatAI = {
    ...
    // 🔥 v8.4: Both branches now have proper combat targeting
    if (PATROL_VCOMAI_Active) then {
        // VCOMAI handles most AI behavior, but we still need to:
        // 1. Monitor ammo
        // 2. Force reveal enemies so VCOMAI can engage them
        private _lastAmmo = 0;
        private _lastCombat = 0;
        
        while {alive _u} do {
            sleep 5;  // Check more frequently
            if (!(_u call DEFENDER_fnc_isUnitValid)) exitWith {};

            // 🔥 v8.4: Active enemy scanning - reveal to VCOMAI
            private _enemies = _u nearEntities [["CAManBase", "LandVehicle", "Air"], DETECT_RAD] select {
                !isNull _x && {[_u, _x] call DEFENDER_fnc_isValidTarget}
            };
            
            if (count _enemies > 0) then {
                private _enemy = _enemies select 0;
                
                // Reveal enemy to unit and group
                _u reveal [_enemy, 4.0];
                (group _u) reveal [_enemy, 4.0];
                
                // Set combat mode
                (group _u) setBehaviour "COMBAT";
                (group _u) setCombatMode "RED";
                
                // Force engagement every 5 seconds
                if (time > _lastCombat + 5) then {
                    _lastCombat = time;
                    _u doTarget _enemy;
                    _u doFire _enemy;
                    (units group _u) commandTarget _enemy;
                };
            };
        ...
        while {alive _u} do {
            sleep (3 + ([0, 2] call BIS_fnc_randomNum));
            if (!(_u call DEFENDER_fnc_isUnitValid)) exitWith {};
            ...
            if (!isNull _enemy && {alive _enemy}) then {
                ...
                // 🔥 v8.4: ALWAYS reveal and target enemy
                _u reveal [_enemy, 4.0];
                _g reveal [_enemy, 4.0];
                
                // 🔥 v8.4: Force targeting every loop
                if (time > _lastTarget + 3) then {
                    _lastTarget = time;
                    _u doTarget _enemy;
                    _u doFire _enemy;
                    _u doWatch _enemy;
                    ...
                    {
                        if (alive _x) then {
                            _x reveal [_enemy, 4.0];
                            _x doTarget _enemy;
                            _x doFire _enemy;
                        };
                    } forEach (units _g);
                    (units _g) commandTarget _enemy;
                };

                // Grenade use at close range
