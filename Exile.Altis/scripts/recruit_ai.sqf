...
diag_log "[AI RECRUIT] ========================================";
diag_log "[AI RECRUIT] Starting v7.39 (Faction Fix + Combat Engagement)...";
diag_log "[AI RECRUIT] 🔥 FIX: Reinforced faction hostility for INDEPENDENT vs EAST";
diag_log "[AI RECRUIT] ========================================";

// ============================================
// 🔥 v7.40: CORRECT FACTION RELATIONSHIP FIX
// INDEPENDENT (Recruit AI) is FRIENDLY to players (RESISTANCE/CIVILIAN)
// INDEPENDENT is HOSTILE to enemies (EAST/WEST)
// ============================================
[] spawn {
    sleep 3; // Wait for A3XAI to initialize
    
    // INDEPENDENT (Recruit AI) hostile to EAST (A3XAI/DyCE enemies)
    INDEPENDENT setFriend [EAST, 0];
    EAST setFriend [INDEPENDENT, 0];
    
    // INDEPENDENT (Recruit AI) hostile to WEST (Zombies)
    INDEPENDENT setFriend [WEST, 0];
    WEST setFriend [INDEPENDENT, 0];
    
    // INDEPENDENT (Recruit AI) FRIENDLY to RESISTANCE (Players + Patrols)
    INDEPENDENT setFriend [RESISTANCE, 1];
    RESISTANCE setFriend [INDEPENDENT, 1];
    
    // INDEPENDENT friendly to CIVILIAN (players in Exile default)
    INDEPENDENT setFriend [CIVILIAN, 1];
    CIVILIAN setFriend [INDEPENDENT, 1];
    
    diag_log "[AI RECRUIT] ✅ v7.40 Faction relationships set:";
    diag_log "[AI RECRUIT]    INDEPENDENT vs EAST = HOSTILE (0) - A3XAI/DyCE enemies";
    diag_log "[AI RECRUIT]    INDEPENDENT vs WEST = HOSTILE (0) - Zombies";
    diag_log "[AI RECRUIT]    INDEPENDENT vs RESISTANCE = FRIENDLY (1) - Players/Patrols";
    diag_log "[AI RECRUIT]    INDEPENDENT vs CIVILIAN = FRIENDLY (1) - Players";
    
    // Verify relationships
    private _eastHostile = INDEPENDENT getFriend EAST;
    private _westHostile = INDEPENDENT getFriend WEST;
    private _resFriendly = INDEPENDENT getFriend RESISTANCE;
    diag_log format ["[AI RECRUIT] Verification: INDEP->EAST=%1 (want 0), INDEP->WEST=%2 (want 0), INDEP->RES=%3 (want 1)", _eastHostile, _westHostile, _resFriendly];
};
...
RECRUIT_fnc_FSM_AnalyzeThreat = {
    ...
    private _threats = _unit nearEntities [["CAManBase"], _maxDist] select {
        alive _x && {!isPlayer _x} && {
            ...
            private _isPatrol = _x getVariable ["PatrolAI_unit", false];

            // Skip other recruit AI and friendly patrol AI
            if (_isRecruit || _isPatrol) exitWith {false};

            // 🔥 v7.40: Force-target A3XAI/DyCE units regardless of reported side
            if (_isA3XAI || _isDyCE) exitWith {true};

            // v7.40: Only EAST (A3XAI) and WEST (Zombies) are enemies
            // RESISTANCE is FRIENDLY (players + patrols)
            (_enemySide == EAST) ||
    ...
    // 🔥 v7.40: Force detection of EAST + WEST units within 200m regardless of knowledge
    // Do NOT target RESISTANCE - they are friendly patrols!
    private _nearbyEnemies = _unit nearEntities [["CAManBase"], 200] select {
        alive _x && {!isPlayer _x} && {
            ...
            private _isRecruit = _x getVariable ["ExileRecruited", false];
            private _isPatrol = _x getVariable ["PatrolAI_unit", false];
            !_isRecruit && !_isPatrol && (_isA3XAI || _isDyCE || side _x == EAST || side group _x == EAST || side _x == WEST)
        }
    };
...
RECRUIT_fnc_FSM_ExecuteState = {
    ...
    switch (_state) do {
        ...
        case FSM_STATE_COMBAT: {
            ...
            // 🔥 v7.39: FORCE GROUP COMBAT MODE - Override any player settings
            _playerGroup setCombatMode "RED";
            _playerGroup setBehaviour "COMBAT";
            _playerGroup enableAttack true;
            
            // 🔥 v7.39: ENSURE GROUP IS HOSTILE TO EAST
            // This is critical - set at group level every combat loop
            (group _unit) setVariable ["combatMode", "RED"];
            ...
            // 🔥 v7.40: AGGRESSIVE ENGAGEMENT - Force attack on EAST/WEST units only
            // RESISTANCE is FRIENDLY (players/patrols) - do NOT attack!
            private _closestThreat = _threatInfo select 1;
            if (!isNull _closestThreat && alive _closestThreat) then {
                // 🔥 v7.40: Simplified side check - EAST and WEST are enemies
                private _threatSide = side _closestThreat;
                private _threatGroupSide = side group _closestThreat;
                private _unitSide = side _unit;
                
                // 🔥 v7.40: Direct enemy check - EAST (A3XAI) and WEST (Zombies) are hostile
                // RESISTANCE is FRIENDLY - do NOT attack!
                private _isEnemy = (_threatSide == EAST) || (_threatGroupSide == EAST) || 
                                   (_threatSide == WEST) || (_threatGroupSide == WEST);
                
                // Also check via variables in case side is wrong
                private _isA3XAI = _closestThreat getVariable ["A3XAI_unit", false];
                private _isDyCE = _closestThreat getVariable ["DyCE_unit", false];
                private _isPatrol = _closestThreat getVariable ["PatrolAI_unit", false];
                
                // Force enemy if A3XAI/DyCE, force friendly if patrol
                if (_isA3XAI || _isDyCE) then { _isEnemy = true; };
                if (_isPatrol) then { _isEnemy = false; };

                // 🔥 v7.41: ONLY ENGAGE IF ACTUALLY AN ENEMY
                if (_isEnemy) then {
                    // Reveal enemy to ALL AI in group at max knowledge
                    _unit reveal [_closestThreat, 4.0];

                    // Force entire group to engage this target
                    _playerGroup reveal [_closestThreat, 4.0];
                    
                    // Set enemy as known target for entire group
                    {
                        if (!isPlayer _x && alive _x) then {
                            _x reveal [_closestThreat, 4.0];
                            _x doTarget _closestThreat;
                            _x doFire _closestThreat;
                        };
                    ...
                    // Use commandTarget for group-level engagement
                    (units group _unit) commandTarget _closestThreat;
                    (units group _unit) commandFire _closestThreat;

                    // Ensure AI is actually aiming and can shoot
                    _unit lookAt _closestThreat;

                    // Check if AI has weapon and force weapon fire
                    ...
                    // Enhanced debug log for targeting issues
                    diag_log format ["[AI RECRUIT COMBAT] %1 engaging %2 | Side: %3 | IsEnemy: true | Dist: %4m",
                        name _unit, typeOf _closestThreat, _threatSide, round (_unit distance _closestThreat)];
                } else {
                    // Not an enemy - clear target if we accidentally targeted a friendly
                    if (assignedTarget _unit == _closestThreat) then {
