...
private _CFG = [
    ...
    ["resurrectA3XAI", false],           // If true, allow A3XAI deaths to spawn zombies
    ["minPlayerDist", 10],
...
addMissionEventHandler ["EntityKilled", {
    ...
    // v3.8: Optional – skip resurrection for A3XAI units to keep missions alive
    private _resurrectA3XAI = ["resurrectA3XAI"] call RMG_Ravage_get;

    if (_isA3XAI && {!_resurrectA3XAI}) exitWith {
        if (_debug) then {
            diag_log "[RMG:Ravage] A3XAI death detected but resurrection disabled for A3XAI - no zombie spawned.";
        };
    };
